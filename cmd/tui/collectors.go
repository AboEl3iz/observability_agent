...
// ─── Adapters: collector samples → TUI rows ───────────────────────────────────

func isContainer(name string) bool {
	return strings.HasPrefix(name, "docker:") || strings.HasPrefix(name, "cri:")
}

func applyTopCPU(s []CpuRow, n int) []CpuRow {
	if n > 0 && len(s) > n {
		return s[:n]
	}
	return s
}
func applyTopMem(s []MemRow, n int) []MemRow {
	if n > 0 && len(s) > n {
		return s[:n]
	}
	return s
}
func applyTopIO(s []IoRow, n int) []IoRow {
	if n > 0 && len(s) > n {
		return s[:n]
	}
	return s
}
func applyTopNet(s []NetRow, n int) []NetRow {
	if n > 0 && len(s) > n {
		return s[:n]
	}
	return s
}
func applyTopSys(s []SysRow, n int) []SysRow {
	if n > 0 && len(s) > n {
		return s[:n]
	}
	return s
...
func cpuRows(samples []collector.CpuSample, cfg displayConfig) []CpuRow {
	var rows []CpuRow
	for _, s := range samples {
		if cfg.containersOnly && !isContainer(s.ContainerName) {
			continue
		}
	...
	return applyTopCPU(rows, cfg.topN)
}

func memRows(samples []collector.MemSample, cfg displayConfig) []MemRow {
	var rows []MemRow
	for _, s := range samples {
		if cfg.containersOnly && !isContainer(s.ContainerName) {
			continue
	...
	return applyTopMem(rows, cfg.topN)
}

func ioRows(samples []collector.IoSample, cfg displayConfig) []IoRow {
	var rows []IoRow
	for _, s := range samples {
		if cfg.containersOnly && !isContainer(s.ContainerName) {
			continue
	...
	return applyTopIO(rows, cfg.topN)
}

func netRows(summaries []collector.NetSummary, cfg displayConfig) []NetRow {
	var rows []NetRow
	...
	return applyTopNet(rows, cfg.topN)
}

func sysRows(summaries []collector.SyscallSummary, cfg displayConfig) []SysRow {
	var rows []SysRow
	for _, s := range summaries {
		if cfg.containersOnly && !isContainer(s.ContainerName) {
			continue
	...
	return applyTopSys(rows, cfg.topN)
}
